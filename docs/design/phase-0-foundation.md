...
- **Data.** Natural Earth 1:50m admin-0 countries and populated places (public domain), stored in `Tools/MapSource`. IDs are ISO 3166-1 alpha-3 from `ISO_A3_EH`; the six territories without one keep the Natural Earth `ADM0_A3` code and are flagged. Every territory on Earth is a `MapCountry`; only the ten simulated ones also have a `CountryDefinition`, joined by ISO3.
- **Pipeline.** `Nation.Core.Map.Import.MapBuildPipeline` is engine-free: GeoJSON → Equal Earth projection → Douglas-Peucker at three tolerances (far, mid, near) → ear-clipping triangulation → validation → `MapCatalogSerializer` binary (730 KB). The Editor menu and the Mono command line call the same code and produce identical bytes. Holes are not carved; enclaves draw above their hosts by area order and a per-country depth step.
- **Rendering.** One combined vertex-colored land mesh and one border mesh per zoom band (three each), an ocean quad and a graticule: four objects drawn per frame, seven with a selection. Thematic layers recolor the vertex arrays; no geometry changes at runtime. Labels and capitals are pooled UI Toolkit elements repositioned only when the camera reports movement.
